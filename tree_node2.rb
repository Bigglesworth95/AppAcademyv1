...
module Searchable
  ...
  # def bfs2(queue, target_value= nil, &prc) #this works. but I use too many parameters so RSPEC fails I think.
  #   raise 'error' if [target_value, prc].none?
  #   return nil if !queue[0].is_a?(PolyTreeNode)
  #   prc ||= Proc.new {|node| node == target_value}
...
parent_index = 0
 ...
 p nodes
#p nodes.first.bfs2([nodes.first], 'x') #issue: we need it to search for the value specifically rather than the node.
p nodes.first.bfs('e')
